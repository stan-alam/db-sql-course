...
--******************* Sellf Join **************************
-- to self join a table to itself means that each row of the tables is combined with itself and with every other
-- row of the table. The self join can be viewed as a join of two copies of the same table. The table is NOT actually copied,
-- but SQL performs the command as though it were.
--******************************************************
--the syntax of the command for joining a table to itself is the same as that for joining two different tables
-- To distinguish the column names from one another, the table name alias is used, since both the tables have the same name
-- one space is left after the table name and the alias for it is typed

--example
-- details of each emp's manager is just another row in the EMP table whose EMPNO is stored in the MGR column
-- of some other row.
-- to list out the name of the manager with the emp record one will have to join EMP WITH ITSELF

select WORKER.ename, MANAGER.ename FROM emp WORKER, emp MANAGER WHERE WORKER.mgr = MANAGER.empno;

----- SET OPERATORS ----------
--SET operators combinet wo or more queries into one result
-- Datatype of corresponding columns must be the same!
-- UNION -> Rows of first query plus the rows of second query, less duplicate rows
...
--************** UNION *****************************
    ...
    -- only the final queries ends with a semicolon
    -- the number of columns in the queries must be the same and of the same datatype
